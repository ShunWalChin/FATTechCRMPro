...
CAPACIDADES = [
    ...
    ("COMERCIAL/Leads", "Deduplicacao", rota("/contacts/duplicates"), "deteccao na criacao + GET /contacts/duplicates + mesclagem que reponta o historico"),
    ...
    ("COMERCIAL/Propostas", "Impostos internos", codigo("sales_operations.py", "tax_cents"), "tax_cents entra no total; o sistema nao calcula tributo nem emite documento fiscal"),
    ("COMERCIAL/Propostas", "Validade", codigo("sales_operations.py", "valid_until"), "valid_until, recusada se no passado"),
    ...
    ("COMERCIAL/Propostas", "Aprovacao em multiplos niveis", codigo("approval_chain.py", "def decidir"), "PARCIAL — a cadeia existe e serve contratos; propostas ainda nao a usam"),
    ...
    ("COMERCIAL/Propostas", "Vinculo com cliente", codigo("sales_operations.py", "\"contact_id\": deal.data"), "contact_id e company_id gravados na criacao"),
    ("COMERCIAL/Propostas", "Geracao assistida por IA", False, "AUSENTE"),

    # --------------------------------------------------------- COMERCIAL > CONTRATOS
    ("COMERCIAL/Contratos", "Modelos contratuais", campo("contract_templates", "body", "variables"), "kind contract_templates com corpo, variaveis e niveis"),
    ("COMERCIAL/Contratos", "Variaveis", codigo("contracts.py", "def resolver_variaveis"), "lidas de empresa, contato, negocio, proposta ou digitadas; sem valor vira marca visivel"),
    ("COMERCIAL/Contratos", "Versoes", rota("/revisions"), "contract_revisions append-only guarda o texto inteiro de cada revisao"),
    ("COMERCIAL/Contratos", "Status", codigo("contracts.py", "TRANSICOES"), "oito estados com transicoes declaradas"),
    ("COMERCIAL/Contratos", "Vigencia", codigo("contracts.py", "starts_on"), "inicio e fim; o fim calculado do modelo se declara aproximado"),
    ("COMERCIAL/Contratos", "Renovacao", rota("/renew"), "estende a vigencia, conta renovacoes e grava revisao"),
    ("COMERCIAL/Contratos", "Responsaveis", codigo("contracts.py", "owner_id"), "owner_id e signatarios com nome, e-mail e papel"),
    ("COMERCIAL/Contratos", "Aprovacoes", codigo("approval_chain.py", "def decidir"), "niveis em ordem, patente minima e a mesma pessoa nao decide dois"),
    ("COMERCIAL/Contratos", "Anexos", False, "PARCIAL — o contrato guarda referencias em documents; nao ha armazenamento de arquivo"),
    ("COMERCIAL/Contratos", "Assinatura externa preparada", rota("/signature"), "caminho pronto; sem provedor recusa com 503 e audita a recusa"),
    ("COMERCIAL/Contratos", "Alertas de vencimento", codigo("services.py", "contract_expiring"), "derivado na leitura, com vencido e vencendo separados"),
    ("COMERCIAL/Contratos", "Vinculo com proposta", rota("/from-proposal"), "so proposta aceita vira contrato, e so uma vez"),
    ("COMERCIAL/Contratos", "Vinculo com cliente", codigo("contracts.py", "company_id"), "empresa ou contato obrigatorio na criacao"),
    ("COMERCIAL/Contratos", "Auditoria completa", codigo("contracts.py", "audit_event"), "criacao, edicao, cada transicao, cada decisao e a recusa de assinatura"),
    ...
    ("COMERCIAL/Produtos", "Custo", campo("products", "cost_cents"), "products.cost_cents; zero significa nao informado"),
    ("COMERCIAL/Produtos", "Margem", False, "PARCIAL — ha custo e preco; a margem ainda nao e derivada em relatorio"),
    ("COMERCIAL/Produtos", "Recorrencia", campo("products", "recurrence"), "mensal, trimestral, semestral, anual ou nenhuma"),
    ("COMERCIAL/Produtos", "Unidade", campo("products", "unit"), "unidade, hora, mes, projeto, licenca, usuario"),
    ("COMERCIAL/Produtos", "Disponibilidade", campo("products", "status"), "PARCIAL — status ativo/inativo, sem estoque nem disponibilidade"),
    ("COMERCIAL/Produtos", "Pacote", campo("products", "bundle_items"), "lista de outros produtos; nao expande sozinho na proposta"),
    ("COMERCIAL/Produtos", "Versao do produto", campo("products", "catalog_version"), "PARCIAL — numero declarado; nao ha historico de precos"),
    ...
    ("COMERCIAL/Produtos", "Vinculo com contratos", rota("/from-proposal"), "PARCIAL — chega ao contrato pela proposta, nao direto"),
    ...
    ("COMERCIAL/Clientes", "Contratos do cliente", codigo("contracts.py", "company_id"), "PARCIAL — filtro por empresa e contato; a visao 360 ainda nao os traz"),
    ...
    ("CLIENTES/Cadastro", "Campos personalizados", campo("custom_fields", "entity", "key", "type"), "kind custom_fields; valores em data.custom, conferidos no servidor"),
    ...
    ("CLIENTES/Cadastro", "Campo personalizado: tipo", campo("custom_fields", "type"), "onze tipos, cada um com conversao e checagem propria"),
    ("CLIENTES/Cadastro", "Campo personalizado: obrigatorio", campo("custom_fields", "required"), "exigido de pessoa; a captura publica segue passando"),
    ("CLIENTES/Cadastro", "Campo personalizado: opcoes", campo("custom_fields", "options"), "escolha unica e multipla, com opcoes distintas"),
    ("CLIENTES/Cadastro", "Campo personalizado: validacao", campo("custom_fields", "validation"), "minimo, maximo, tamanho e expressao regular"),
    ("CLIENTES/Cadastro", "Campo personalizado: visibilidade", codigo("custom_fields.py", "def ocultar"), "campo restrito nao sai da API, nao apenas da tela"),
    ("CLIENTES/Cadastro", "Campo personalizado: ordem", campo("custom_fields", "position"), "position ordena o formulario"),
    ("CLIENTES/Cadastro", "Campo personalizado: historico", False, "PARCIAL — a trilha registra a edicao do registro; nao ha historico por valor"),
    ("CLIENTES/Cadastro", "Campo personalizado: permissao", campo("custom_fields", "editable_by"), "editable_by recusa com 403 no servidor"),
    ...
    ("PARIDADE/Vendas", "Aprovacao", codigo("approval_chain.py", "def decidir"), "cadeia em niveis com segregacao de funcoes"),
    ("PARIDADE/Vendas", "Cotacao", rota("/sales/proposals"), "propostas com preco congelado"),
    ("PARIDADE/Vendas", "Contratos", rota("/contracts"), "subsistema completo: modelo, revisoes, cadeia, vigencia e renovacao"),
    ...
    ("IA/Agente", "Instrucoes", campo("agents", "description", "role"),
     "PARCIAL — persona e papel declarados; nao ha prompt de sistema versionado"),
    ("IA/Agente", "Base de conhecimento", campo("knowledge", "content"), "PARCIAL — kind knowledge existe, sem vinculo com agente"),
    ("IA/Agente", "Escopo de dados", rota("/agent/identity"),
     "identidade propria com chave de escopo derivado; chave nunca faz operacao administrativa"),
    ("IA/Agente", "Modelo", campo("agents", "model"), "PARCIAL — o modelo e declarado na configuracao; nenhum executor o usa ainda"),
    ("IA/Agente", "Orcamento", campo("agents", "budget_cents", "spent_cents"), "orcamento e gasto declarados em centavos"),
    ("IA/Agente", "Limite de tokens", campo("agents", "budget_month_cents", "max_actions_per_hour"),
     "PARCIAL — teto de gasto e de acoes declarado e recusado quando ausente; o portao que o aplica e o E2"),
    ("IA/Agente", "Autonomia", campo("agents", "autonomy"), "agents.autonomy"),
    ("IA/Agente", "Ferramentas permitidas", rota("/agent/tools"),
     "catalogo derivado de RESOURCES, com escopo, classe e reversibilidade por ferramenta"),
    ("IA/Agente", "Acoes proibidas", codigo("agent_tools.py", "FORA_DO_ALCANCE"),
     "dominios fora do alcance declarados com motivo; exclusao marcada irreversivel em todo dominio"),
    ("IA/Modo", "Sugestao", codigo("agent_gate.py", "suggested"),
     "escrita em modo sugestao vira rascunho; uma pessoa aplica e a trilha registra o nome dela"),
    ("IA/Modo", "Execucao interna", codigo("agent_gate.py", "execucao_interna"),
     "cria, edita e move dentro do sistema, com teto de acoes e de gasto aplicados"),
    ("IA/Modo", "Copiloto com revisao humana", tela("agente"),
     "tela lista o rascunho, mostra a justificativa e exige uma pessoa para aplicar"),
    ("IA/Modo", "Despacho por evento", rota("/agent/runs/claim"),
     "consumidor openclaw do Core-Engine materializa a corrida; o agente reclama e nao e empurrado"),
    ("IA/Agente", "Gatilhos", campo("agents", "triggers"),
     "evento declarado na configuracao acorda o agente; sem gatilho ativo nada e despachado"),
    ("IA/Guardrail", "Laco de realimentacao", codigo("agent_dispatch.py", "startswith(\"agent.\")"),
     "evento do proprio agente nao o acorda; o laco e cortado na origem, nao no quinto salto"),
    ("IA/Guardrail", "Portao de execucao", rota("/agent/act"),
     "oito checagens em ordem fixa; toda tentativa vira passo gravado, inclusive a recusa"),
    ("IA/Guardrail", "Registro de recusa", codigo("agent_gate.py", "refusal_reason"),
     "a tentativa recusada fica gravada com motivo em tabela append-only"),
    ("IA/Agente", "Historico de execucao", rota("/agent/runs"),
     "corrida com justificativa e passos, inclusive os recusados, e o evento que a disparou"),
    ("IA/Agente", "Desligar o agente", rota("/agent/identity/{agent_id}"),
     "revoga toda chave ativa e desativa a identidade; a via generica nao alcanca um root"),
    ...
    ("SEGURANCA", "Trilha verificavel por terceiro", rota("/audit/verify"),
     "cadeia de hash por organizacao; a verificacao reporta quantas linhas conferiu"),
