...
export function WorkQueue(){
 ...
 {!loading&&!error&&(data.items.length===0?<div className="crm-panel"><EmptyState title="Nenhuma tarefa nesta seleção." description="Ajuste os filtros ou registre o próximo passo da sua operação." action={write?'Nova tarefa':undefined} onAction={()=>setEditing(null)}/></div>:<div className="queue-list">{data.items.map(item=><article className={`queue-row${item.status==='done'?' is-done':''}`} key={item.id}>
 <div className="queue-main">
  <div className="queue-head"><h2>{textValue(item.title)}</h2><StatusBadge value={item.status}/><StatusBadge value={item.priority}/></div>
  {textValue(item.description)&&<p className="queue-description">{textValue(item.description)}</p>}
  <dl className="queue-facts"><div><dt>Prazo · UTC</dt><dd>{deadline(item.due_date)}</dd></div><div><dt>Responsável</dt><dd>{textValue(item.owner_name)||(item.owner_id?'Responsável indisponível':'Sem responsável')}</dd></div>{Boolean(item.contact_name)&&<div><dt>Contato</dt><dd><Link href={recordHref('contacts',textValue(item.contact_id))}>{textValue(item.contact_name)}</Link></dd></div>}{Boolean(item.deal_name)&&<div><dt>Oportunidade</dt><dd><Link href={recordHref('deals',textValue(item.deal_id))}>{textValue(item.deal_name)}</Link></dd></div>}</dl>
 </div>
 {write&&<div className="queue-actions"><Button variant="secondary" isDisabled={Boolean(busy)} onPress={()=>setEditing(item)} aria-label={`Editar ${textValue(item.title)}`}>Editar</Button><Button className="fat-button" isDisabled={Boolean(busy)} onPress={()=>toggle(item)} aria-label={`${item.status==='done'?'Reabrir':'Concluir'} ${textValue(item.title)}`}><Check size={16}/>{busy===item.id?'Salvando…':item.status==='done'?'Reabrir':'Concluir'}</Button></div>}
 </article>)}</div>)}
